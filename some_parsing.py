...
def get_today():
    today = asyncio.wait(get_rsp("today"))
    return today
    
def get_tomorrow():
    tomorrow = asyncio.wait(get_rsp("tomorrow"))
    return tomorrow

async def waiter_checker():
    while(True):
        print("Считаю сколько спать")
        time_to_sleep = (datetime.datetime.now().replace(hour = 9, minute=30, second=0, microsecond=0) + datetime.timedelta(1) - datetime.datetime.now())
...
def checker():
    ...
    try:
        responce = requests.get(f"https://rsp.chemk.org/4korp/tomorrow.htm")
        responce.encoding = 'windows-1251'
        contents = responce.text
        soup = BeautifulSoup(contents, "html.parser")
        div = soup.find("div", class_="Section1")
...
async def start(message: types.Message):
    ...
    buttons = ["/Today", "/Tomorrow", "/Subscribe"]
    keyboard.add(*buttons)
    asyncio.create_task(dump_logs(f"Issued \"start\" from {message.from_user.username} in {datetime.datetime.fromtimestamp(message.date)}\n"))
...
async def cmd_start(message: types.Message):
    asyncio.create_task(dump_logs(f"Issued \"Today\" from {message.from_user.username} in {datetime.datetime.fromtimestamp(message.date)}\n"))
    # try:
            ...
            with open ( "config.json" , "w" ) as config:
                ...
                if(chat_id in ids):
                    try:
                        ...
                        asyncio.create_task(bot.reply_to(message, "[WIP]Не получилось отписаться от обновлений расписания"))
                    else:
                        asyncio.create_task(bot.reply_to(message, "[WIP]Успешно получилось отписаться от обновление расписания"))
                else:
                    ids.append(chat_id)
                    asyncio.create_task(bot.reply_to(message, "[WIP]Успешно подписан на обновление расписания"))
...
async def cmd_start(message: types.Message):
    ...
    asyncio.create_task(dump_logs(f"Issued \"Tomorrow\" from {message.from_user.username} in {datetime.datetime.fromtimestamp(message.date)}\n"))
    await bot.reply_to(message, rasp)
